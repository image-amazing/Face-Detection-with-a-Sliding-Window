...
if ~exist('variables/feature_pos.mat','file')
    features_pos = get_positive_features( train_path_pos, feature_params );

    save('variables/feature_pos.mat','features_pos')
else
    load('variables/feature_pos.mat','features_pos')
end

num_negative_examples = 20000; %Higher will work strictly better, but you should start with 10000 for debugging

if ~exist('variables/feature_neg.mat','file')
    features_neg = get_random_negative_features( non_face_scn_path, feature_params, num_negative_examples);
    save('variables/feature_neg.mat','features_neg')
else
    load('variables/feature_neg.mat','features_neg')
end
...
% sizeOfNewNeg = 1000;
% while sizeOfNewNeg>100
%     
%     if(size(features_neg,1)>80000)
%         break;
%     end
%     [features, bboxes_1, confidences_1, image_ids_1] = run_detector(non_face_scn_path, w, b, feature_params, 'hardmining');
%     sizeOfNewNeg = size(features,1);
%     fprintf('\n\n\n New Negative Features found: %d\n', sizeOfNewNeg);
%     
%     num_positive_examples = size(features_pos,1);
...
% save('variables/hardMining.mat','features_neg','dataset','labels','w','b');

if hardMining == 1
    if ~exist('variables/hardMining.mat','file')
        [features, bboxes_1, confidences_1, image_ids_1] = run_detector(non_face_scn_path, w, b, feature_params, 'hardmining');
        ...
        load('variables/hardMining.mat','features_neg','dataset','labels','w','b')
    end
end
...
[features, bboxes, confidences, image_ids] = run_detector(test_scn_path, w, b, feature_params,'test');
